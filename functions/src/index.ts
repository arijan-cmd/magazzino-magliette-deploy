...
export const sendSaleMail = onRequest(async (req, res) => {
  if (req.method !== 'POST') {
  ...
  if (!smtpHost || !smtpUser || !smtpPass || !smtpTo || !smtpFrom) {
    ...
    res.status(200).json({
      ...
      message: 'SMTP non configurato. Imposta SMTP_HOST, SMTP_USER, SMTP_PASS, SMTP_TO nel file functions/.env.',
    });
    return;
  }
  ...
  try {
    ...
    res.status(500).json({ success: false, message: "Errore durante l'invio dell'email: " + message });
  }
});
